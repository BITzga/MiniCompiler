// Generated from D:/AntlrToJson/src/main/resources\C.g4 by ANTLR 4.9.1
...
public class CBaseListener implements CListener {
	ParseTreeProperty property;
	ASTCompilationUnit compilationUnit;
	CBaseListener(){
		property = new ParseTreeProperty();
		compilationUnit = new ASTCompilationUnit();
		compilationUnit.items = new ArrayList<>();
	...
	@Override public void enterDeclaration(CParser.DeclarationContext ctx) {
		...
		Iterator iterator = ctx.declarationSpecifiers().children.iterator();
		while (iterator.hasNext()){
			CParser.DeclarationSpecifiersContext context = (CParser.DeclarationSpecifiersContext)iterator.next();
			builder.addSpecfiers(property.get(context));
		}
		property.put(ctx,builder.build());
	}
	/**
	 * {@inheritDoc}
	 *
...
//	private Map<String,ParserRuleContext> visit = new HashMap<>();
//	public ParserRuleContext dfs(ParserRuleContext node){
//		if(node==null)
//			return null;
//		if(visit.get(CParser.ruleNames[node.getRuleIndex()])!=null){
//			return visit.get(CParser.ruleNames[node.getRuleIndex()]);
//		}
//		ParserRuleContext root = new ParserRuleContext();
//		root.children  = new ArrayList<>();
//		visit.put(CParser.ruleNames[node.getRuleIndex()],root);
//		for(int i=0;i<node.getChildCount();i++){
//			root.children.add(dfs(node.getChild(ParserRuleContext.class,i)));
//		}
//		return root;
//	}
	/**
	...
	@Override public void enterTranslationUnit(CParser.TranslationUnitContext ctx) {
		...
		Iterator iterator = ctx.externalDeclaration().iterator();
		while(iterator.hasNext()){
			Object child = property.get((CParser.ExternalDeclarationContext)iterator.next());
			builder.addNode(child);
		}
		this.compilationUnit = builder.build();
		property.put(ctx,this.compilationUnit);
	}
	/**
	 * {@inheritDoc}
	 *
